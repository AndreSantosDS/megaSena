...
btnSortear.addEventListener("click", function(){
    container1.querySelectorAll(".numeros-soteados, .voceGanhou, .vocePerdeu").forEach(e => e.remove());
    numeros = [];
    sortearNumeros(6, 1, 60);
    numerosSorteados();

    const acertos = numerosEscolhidosPeloUsuario.filter(n => numeros.includes(n));
    ...
    if(acertos.length === 6){
        ...
        vocePerdeu.textContent = `Infelizmente não foi dessa vez. Você acertou ${acertos.length} número(s).`;
